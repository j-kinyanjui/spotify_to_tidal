...
def tidal_search(spotify_track_and_cache, tidal_session):
    ...
    for track in tidal_session.search(simple(spotify_track['name']) + ' ' + simple(spotify_track['artists'][0]['name']),
                                      models=[tidalapi.media.Track])['tracks']:
        if match(track, spotify_track):
            return track


def get_tidal_playlists_dict(tidal_sess):
    # a dictionary of name --> playlist
    current_tidal_playlists = tidal_sess.user.playlists()
    output = {}
    for playlist in current_tidal_playlists:
...
def repeat_on_request_error(function, *args, remaining=5, **kwargs):
    ...
    try:
        ...
        if not remaining:
            ...
            sys.exit(1)
        sleep_schedule = {5: 1, 4: 10, 3: 60, 2: 5 * 60,
                          1: 10 * 60}  # sleep variable length of time depending on retry number
...
class TidalPlaylistCache:
    ...
    def search(self, spotify_sess, sp_playlist):
        """ Add the cached tidal track where applicable to a list of spotify tracks """
        results = []
        cache_hits = 0
        work_to_do = False
        spotify_tracks = get_tracks_from_spotify_playlist(spotify_sess, sp_playlist)
...
def get_user_playlist_mappings(spotify_sess, tidal_sess, current_config):
    results = []
    spotify_playlists = get_playlists_from_spotify(spotify_sess, current_config)
    tidal_playlists = get_tidal_playlists_dict(tidal_sess)
    for spotify_playlist in spotify_playlists:
        results.append(pick_tidal_playlist_for_spotify_playlist(spotify_playlist, tidal_playlists))
    return results


def get_playlists_from_spotify(spotify_sess, current_config):
    # get all the user playlists from the Spotify account
    playlists = []
    spotify_results = spotify_sess.user_playlists(current_config['spotify']['username'])
    exclude_list = set([x.split(':')[-1] for x in current_config.get('excluded_playlists', [])])
    while True:
        for spotify_playlist in spotify_results['items']:
            if spotify_playlist['owner']['id'] == current_config['spotify']['username'] and not spotify_playlist[
                                                                                            'id'] in exclude_list:
        ...
        if spotify_results['next']:
            spotify_results = spotify_sess.next(spotify_results)
        else:
            break
    return playlists


def get_playlists_from_config(current_config):
    # get the list of playlist sync mappings from the configuration file
    return [(item['spotify_id'], item['tidal_id']) for item in current_config['sync_playlists']]
...
if __name__ == '__main__':
    ...
    if args.uri:
        ...
        sync_list(spotify_session, tidal_session, get_user_playlist_mappings(spotify_session, tidal_session, config),
                  config)
